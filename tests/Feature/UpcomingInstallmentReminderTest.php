...
class UpcomingInstallmentReminderTest extends TestCase
{
    ...
    public function test_reminder_is_sent_once_exactly_ten_days_before_an_active_plan_installment(): void
    {
        Mail::fake();
        ...
        $due = InstallmentSchedules::create(['booking_id' => $booking->id, 'installment_number' => 1, 'due_date' => today()->addDays(10), 'regular_amount' => 25000, 'balloon_amount' => 0, 'total_due' => 25000, 'paid_amount' => 5000, 'status' => 'partial']);
        $future = InstallmentSchedules::create(['booking_id' => $booking->id, 'installment_number' => 2, 'due_date' => today()->addDays(11), 'regular_amount' => 25000, 'balloon_amount' => 0, 'total_due' => 25000, 'status' => 'pending']);

        $this->artisan('installments:send-upcoming-reminders')->expectsOutput('1 upcoming installment reminder(s) sent.')->assertSuccessful();
        ...
        $this->assertSame('Installment due in 10 days', $customerUser->notifications()->first()->data['title']);

        $this->artisan('installments:send-upcoming-reminders')->expectsOutput('0 upcoming installment reminder(s) sent.')->assertSuccessful();
        Mail::assertSentCount(1);

        $due->update(['due_date' => today()->addDays(9), 'reminder_sent_at' => now()->subDay()]);
        $this->artisan('installments:send-upcoming-reminders')->assertSuccessful();
        Mail::assertSentCount(2);
        $this->assertTrue($customerUser->notifications()->get()->contains(fn ($notification) => $notification->data['title'] === 'Installment due in 9 days'));

        $future->update(['due_date' => today(), 'reminder_sent_at' => null]);
        $this->artisan('installments:send-upcoming-reminders')->assertSuccessful();
        Mail::assertSentCount(3);
        $this->assertTrue($customerUser->notifications()->get()->contains(fn ($notification) => $notification->data['title'] === 'Installment due today'));
    }
