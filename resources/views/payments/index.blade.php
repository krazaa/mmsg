<x-app-layout>
    <div class="py-5 sm:py-8">
        <div class="mx-auto max-w-7xl space-y-5 px-3 sm:px-4">
            <section class="admin-command-card admin-command-summary compact-command-card p-5 text-white shadow-2xl">
                <div class="absolute -right-16 -top-20 h-64 w-64 rounded-full"></div>
                <div class="relative grid gap-4 lg:grid-cols-[minmax(250px,.72fr)_minmax(0,1.65fr)] lg:items-center"><div><div class="admin-command-kicker text-[10px] font-black uppercase tracking-[.18em]">Payment management</div><h1 class="mt-2 text-3xl font-black">Payments</h1><p class="mt-2 max-w-md text-sm leading-5 text-emerald-100/80">Review customer receipts, verify transactions and monitor collected funds.</p></div><div class="admin-command-stat-grid grid grid-cols-2 gap-2.5 lg:grid-cols-4">@foreach([['All payments',$summary['total'],'Total receipts','bg-emerald-400',false],['Pending review',$summary['pending'],'Awaiting verification','bg-amber-400',false],['Verified',$summary['verified'],'Approved receipts','bg-cyan-400',false],['Amount received',$summary['received'],'Verified funds','bg-violet-400',true]] as [$label,$value,$hint,$accent,$currency])<div class="admin-command-stat rounded-2xl border backdrop-blur"><div class="flex items-center gap-2 text-[9px] font-black uppercase tracking-wider text-emerald-200 sm:text-[10px]"><span class="h-2 w-2 rounded-full {{ $accent }}"></span>{{ $label }}</div><div class="mt-2 break-words text-xl font-black sm:text-2xl">{{ $currency ? 'Rs '.number_format($value) : number_format($value) }}</div><div class="mt-1 text-[10px] text-emerald-100/65">{{ $hint }}</div></div>@endforeach</div></div>
            </section>
