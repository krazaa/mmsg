                ...
                >@csrf @method('PUT')
                    ...
                    @if($payment->installment_schedule_id === null)
                        <label x-show="status === 'verified'" x-cloak class="block text-sm font-semibold text-gray-700">
                            ...
                            <input name="file_no" value="{{ old('file_no', '') }}" :required="status === 'verified' && @js($payment->status === 'pending')" maxlength="50" placeholder="Enter the manually assigned file number" class="mt-1.5 w-full rounded-lg border-gray-300 font-mono uppercase">
