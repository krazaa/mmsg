<x-app-layout>
    <div class="relative min-h-screen overflow-hidden bg-gradient-to-br from-indigo-50 via-white to-cyan-50 py-7 dark:from-slate-950 dark:via-slate-950 dark:to-indigo-950/40">
        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6">
            <section class="admin-command-card admin-command-summary mb-6 p-5 text-white shadow-2xl sm:p-8">
                <div class="absolute -right-16 -top-20 h-64 w-64 rounded-full bg-violet-500/20 blur-3xl"></div>
                <div class="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><div class="admin-command-kicker text-[10px] font-black uppercase tracking-[.22em]">Activity center</div><h1 class="mt-2 text-3xl font-black sm:text-4xl">Alerts & notifications</h1><p class="mt-2 text-sm text-indigo-100/80">Stay on top of bookings, payments and customer activity.</p></div>@if($unreadCount)<form method="POST" action="{{ route('management.notifications.read-all') }}">@csrf<button class="w-full rounded-2xl bg-white px-5 py-3 text-sm font-black text-indigo-950 sm:w-auto">✓ Mark all read</button></form>@endif</div>
                <div class="admin-command-stat-grid mt-4 grid grid-cols-2 gap-2.5 lg:grid-cols-4">@foreach([['All alerts',$totalCount,'Total notifications','bg-indigo-400'],['Unread',$unreadCount,'Need attention','bg-rose-400'],['Read',max(0,$totalCount-$unreadCount),'Reviewed alerts','bg-emerald-400'],['Received today',$todayCount,'Latest activity','bg-cyan-400']] as [$label,$value,$hint,$accent])<div class="admin-command-stat rounded-2xl border p-3.5 backdrop-blur sm:p-4"><div class="flex items-center gap-2 text-[9px] font-black uppercase tracking-wider text-indigo-200 sm:text-[10px]"><span class="h-2 w-2 rounded-full {{ $accent }}"></span>{{ $label }}</div><div class="mt-2 text-2xl font-black sm:text-3xl">{{ number_format($value) }}</div><div class="mt-1 text-[10px] text-indigo-100/65 sm:text-xs">{{ $hint }}</div></div>@endforeach</div>
            </section>
            @if(session('success'))
