<x-app-layout>
    <div class="py-5 sm:py-8"><div class="mx-auto max-w-7xl px-3 sm:px-4 space-y-5">
        <section class="admin-command-card admin-command-summary installments-command-card p-5 text-white shadow-2xl">
            ...
            <div class="relative grid gap-4 lg:grid-cols-[minmax(250px,.72fr)_minmax(0,1.65fr)] lg:items-center">
                <div><div class="admin-command-badge inline-flex rounded-lg px-3 py-1.5 text-[10px] font-black uppercase tracking-[.18em]">Payment schedule management</div><h1 class="mt-2 text-3xl font-black tracking-tight">Installments</h1><p class="mt-2 max-w-md text-sm leading-5 text-violet-100/80">Monitor upcoming payments, overdue schedules and customer payment progress.</p></div>
                <div class="admin-command-stat-grid grid grid-cols-2 gap-2.5 lg:grid-cols-4">
                    @foreach([
                        ...
                        <div class="admin-command-stat rounded-2xl border backdrop-blur">
                            ...
                            <div class="mt-1 text-[10px] leading-4 text-violet-100/65">{{ $hint }}</div>
                        </div>
                    @endforeach
                </div>
            </div>
