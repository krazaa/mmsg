<x-app-layout>
    ...
    <div class="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-emerald-50 py-8 dark:from-slate-950 dark:via-slate-950 dark:to-indigo-950/30">
        <div class="mx-auto max-w-4xl space-y-5 px-4 sm:px-6">
            ...
            <form method="POST" action="{{ route('app-settings.update') }}" x-data="{ feeEnabled: @js((bool) old('fee_enabled', $fee['enabled'])), feeType: @js(old('fee_type', $fee['type'])), pinRecovery: @js((bool) old('pin_recovery_enabled', $pinRecoveryEnabled)), showReferral: @js((bool) old('customer_portal_show_referral_code', $showReferralCode)) }" class="overflow-hidden rounded-3xl border border-indigo-100 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900">
                @csrf @method('PUT')
