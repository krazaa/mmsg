...
@php
    $defaultBalloons = isset($package)
        ...
        : [['month' => 12, 'amount' => 150000], ['month' => 24, 'amount' => 250000], ['month' => 36, 'amount' => 350000]];
...
<div class="grid gap-5 sm:grid-cols-2" x-data="{
    ...
    <label class="block text-sm font-medium text-gray-700">Duration (months)
        <input type="number" min="1" max="60" step="1" name="months" x-model.number="months" value="{{ old('months', $package->months ?? 36) }}" required class="mt-1 w-full rounded-md border-gray-300">
            ...
            <template x-for="(balloon, index) in balloons" :key="index">
                <div class="grid grid-cols-[1fr_1fr_auto] items-end gap-3">
                    <label class="block text-sm font-medium text-gray-700">Month
                        <input type="number" min="1" step="1" :max="months" :name="`balloons[${index}][month]`" x-model.number="balloon.month" required class="mt-1 w-full rounded-md border-gray-300" :class="Number(balloon.month) > Number(months) ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : ''">
                    ...
                    <label class="block text-sm font-medium text-gray-700">Amount (Rs)
                        <input type="number" min="0.01" step="0.01" :name="`balloons[${index}][amount]`" x-model.number="balloon.amount" required class="mt-1 w-full rounded-md border-gray-300">
                    </label>
