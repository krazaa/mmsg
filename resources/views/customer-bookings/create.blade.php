    ...
    <div class="min-h-screen bg-gradient-to-b from-indigo-50/80 via-slate-50 to-white py-6 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 sm:py-8"><div class="mx-auto max-w-7xl space-y-8 px-4 sm:px-6">
        ...
        @forelse($projects as $project)
            ...
            <section id="project-{{ $project->id }}" class="scroll-mt-20 space-y-5">
                <div class="flex flex-col gap-4 rounded-2xl border border-gray-100 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:flex-row sm:items-center sm:justify-between"><div><div class="mb-1 text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-300">Available project</div><h3 class="text-2xl font-black text-gray-900 dark:text-white">{{ $project->name }}</h3><p class="mt-1 text-sm text-gray-500 dark:text-gray-300">{{ $project->location }}</p></div><div class="w-full sm:max-w-xs"><div class="mb-2 flex justify-between text-xs font-bold"></div><div class="h-2.5 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700"></div></div></div>
                            ...
                            <form x-data="{ confirmOpen: false, submitting: false, paymentPlan: @js($initialPlan), cashPrice: {{ (float) $package->effective_cash_price }}, installmentPrice: {{ (float) $package->total_price }}, money(value) { return 'Rs ' + Number(value || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) } }" x-ref="bookingForm" method="POST" action="{{ route('customer.bookings.store') }}" @submit.prevent="if (paymentPlan) confirmOpen = true" class="mt-5">
                                ...
                                <div x-show="paymentPlan" x-cloak class="mb-4 overflow-hidden rounded-2xl border" :class="paymentPlan === 'cash' ? 'border-emerald-200 bg-emerald-50/70 dark:border-emerald-900 dark:bg-emerald-950/30' : 'border-indigo-200 bg-indigo-50/70 dark:border-indigo-900 dark:bg-indigo-950/30'">
                                    <div x-show="paymentPlan === 'cash'" x-cloak class="p-4">
                                        ...
                                        <div class="mt-3 grid {{ $singlePlan ? 'grid-cols-1' : 'grid-cols-2' }} gap-2 text-xs"><div class="rounded-xl bg-white/70 p-3 dark:bg-slate-900/50"><span class="block text-[9px] font-bold uppercase text-slate-400">Payment due</span><b class="mt-1 block text-emerald-800 dark:text-emerald-200">Full amount</b></div>@unless($singlePlan)<div class="rounded-xl bg-white/70 p-3 dark:bg-slate-900/50"><span class="block text-[9px] font-bold uppercase text-slate-400">Schedule</span><b class="mt-1 block text-emerald-800 dark:text-emerald-200">No installments</b></div>@endunless</div>
                                    </div>
