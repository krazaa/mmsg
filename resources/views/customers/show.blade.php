    ...
    <div class="mx-auto max-w-7xl space-y-6 px-4">
        ...
        @if($agent)
            ...
            <section class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.55fr)]">
                <div class="overflow-x-auto rounded-xl bg-white shadow-sm"><div class="border-b p-5"><h3 class="font-bold">Commission history</h3><p class="mt-1 text-sm text-gray-500">Lifetime commission earned through verified customer payments.</p></div><table class="w-full min-w-[700px] text-sm"><thead class="bg-gray-50 text-left text-gray-500"><tr><th class="p-4">Date</th><th>Booking / customer</th><th>Level</th><th>Rate</th><th>Amount</th><th>Status</th></tr></thead><tbody>@forelse($agentCommissions as $commission)<tr class="border-t"><td class="p-4">{{ $commission->created_at->format('d M Y') }}</td><td>{{ $commission->booking?->booking_number ?? '—' }}<div class="text-xs text-gray-500">{{ $commission->booking?->customer?->name ?? '—' }}</div></td><td>Level {{ $commission->level }}</td><td>{{ number_format($commission->percentage,2) }}%</td><td class="font-semibold">Rs {{ number_format($commission->amount,2) }}</td><td><span class="rounded-full px-2 py-1 text-xs {{ $commission->status === 'earned' ? 'bg-amber-100 text-amber-700' : ($commission->status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700') }}">{{ ucfirst($commission->status) }}</span></td></tr>@empty<tr><td colspan="6" class="p-8 text-center text-gray-500">No commission history.</td></tr>@endforelse</tbody></table></div>
                <div class="space-y-6">
                    <form method="POST" action="{{ route('agents.payouts.store',$agent) }}" class="rounded-xl bg-white p-5 shadow-sm">@csrf<input type="hidden" name="return_to" value="customer"><h3 class="font-bold">Pay commission</h3><p class="mt-1 text-sm text-gray-500">This records and pays the full payable balance.</p><div class="mt-4 rounded-lg bg-amber-50 p-4"><div class="text-xs text-amber-700">Amount to pay</div><div class="mt-1 text-xl font-bold text-amber-800">Rs {{ number_format($agentSummary->payable,2) }}</div></div><label class="mt-4 block text-sm font-medium">Payment method<select name="payment_method" required class="mt-1 w-full rounded-lg border-gray-300"><option value="bank_transfer">Bank transfer</option><option value="cash">Cash</option><option value="cheque">Cheque</option><option value="easypaisa">Easypaisa</option><option value="jazzcash">JazzCash</option><option value="crypto">Crypto</option></select></label><label class="mt-3 block text-sm font-medium">Reference<input name="transaction_reference" class="mt-1 w-full rounded-lg border-gray-300"></label><label class="mt-3 block text-sm font-medium">Notes<textarea name="notes" rows="2" class="mt-1 w-full rounded-lg border-gray-300"></textarea></label><button @disabled($agentSummary->payable <= 0) class="mt-4 w-full rounded-lg bg-indigo-600 px-4 py-2.5 font-semibold text-white disabled:cursor-not-allowed disabled:bg-gray-300">{{ $agentSummary->payable > 0 ? 'Pay full commission' : 'No commission payable' }}</button></form>
