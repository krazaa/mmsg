...
>
    ...
    <div :class="sidebarExpanded ? 'lg:pl-64' : 'lg:pl-20'" class="min-h-screen transition-[padding] duration-300">
            ...
            <div class="flex items-center gap-2">
                @can('manage notifications')
                    ...
                    <a href="{{ route('management.notifications.index') }}" class="relative grid h-10 w-10 place-items-center rounded-xl text-gray-500 transition hover:bg-indigo-50 hover:text-indigo-600 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white" title="Notifications" aria-label="Notifications">
                        ...
                        @if($unreadNotificationCount)<span class="absolute right-0.5 top-0.5 min-w-4 rounded-full bg-rose-500 px-1 text-center text-[9px] font-black leading-4 text-white ring-2 ring-white dark:ring-gray-800">{{ $unreadNotificationCount > 99 ? '99+' : $unreadNotificationCount }}</span>@endif
                    </a>
                @endcan
        ...
        @isset($header)<header class="border-b border-slate-200 bg-slate-50/80 px-4 py-4 dark:border-slate-800 dark:bg-slate-950 sm:px-6"><div class="admin-command-card mx-auto max-w-7xl rounded-3xl px-5 py-4 sm:px-6 sm:py-5">{{ $header }}</div></header>@endisset
        <main>{{ $slot }}</main>
    </div>
</div>
